...
async function parseHwpxDocument(buffer, options) {
  ...
  const markdown = blocksToMarkdown(structured);
...
function parseHwp5Document(buffer, options) {
  ...
  flatBlocks = splitArticleHeadings(flatBlocks);
  if (docInfo) {
...
function detectHwp5Headings(blocks, docInfo) {
  ...
  for (const block of blocks) {
    ...
    if (level === 0) level = legalStructureLevel(text);
    if (level > 0) {
      block.type = "heading";
      block.level = level;
    }
...
async function parseXlsxDocument(buffer, options) {
  ...
  let pageFilter = null;
  if (options?.pages) {
    const { parsePageRange: parsePageRange2 } = await import("./page-range-LWD2W2OP.js");
...
function linearizeTable(table) {
  ...
  const cellText = (r, c) => normalize2((cells[r]?.[c]?.text ?? "").replace(/<br\s*\/?>/gi, " "));
...
async function parsePdf(buffer, options) {
  ...
  try {
    const mod = await import("./parser-BSWAYAUT.js");
