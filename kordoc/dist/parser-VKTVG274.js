...
function detectTablesInItems(merged, originMap, pageNum) {
...
function mergeXCoverage(items) {
  const intervals = items.map((i) => [i.x, i.x + i.w]).sort((a, b) => a[0] - b[0]);
  const coverage = [];
...
function splitIntoColumnBands(items) {
  ...
  const sortedW = gaps.map((g2) => g2.width).sort((a, b) => a - b);
  ...
  const gutters = gaps.filter((g2) => g2.width >= minGutter).sort((a, b) => a.mid - b.mid);
  if (gutters.length === 0) return [items];
  const cutXs = gutters.map((g2) => g2.mid);
  const bands = Array.from({ length: cutXs.length + 1 }, () => []);
...
async function parsePdfDocument(buffer, options) {
  ...
  try {
    ...
    if (totalChars / Math.max(parsedPageCount, 1) < 10) {
      if (options?.ocr) {
        try {
          const { ocrPages } = await import("./provider-KCE5I742.js");
...
function collapseNumberSpacing(text) {
  return text.replace(/(\d)\s*,\s*(?=\d)/g, "$1,").replace(/(^|\()\s*[-−]\s+(?=\d)/g, "$1-");
}
...
function detectAmendmentTable(items, pageNum) {
  ...
  for (const band of bands) {
    ...
    if (/신.{0,2}구조문\s*대비표/.test(text)) continue;
  ...
  const table = { rows: cells.length, cols: 2, cells, hasHeader: headerSeen };
  return [{ type: "table", table, pageNumber: pageNum, bbox: computeBBox(items, pageNum) }];
}
function findCenterGutter(items, minX, maxX, W) {
  const bins = /* @__PURE__ */ new Map();
  for (const i of items) {
    const b = Math.round(i.x / 10) * 10;
...
function mergeAdjacentTableBlocks(blocks) {
  ...
  for (let i = 1; i < blocks.length; i++) {
    const prev = result[result.length - 1];
    const curr = blocks[i];
    if (prev.type === "table" && curr.type === "table" && prev.table && curr.table && prev.table.cols === curr.table.cols && xOverlapRatio(prev.bbox, curr.bbox) >= 0.5) {
      const merged = {
...
//# sourceMappingURL=parser-VKTVG274.js.map