...
function cellBorderStyle(b) {
  if (!b) return "";
  if (b.top === 1 && b.right === 1 && b.bottom === 1 && b.left === 1) return "";
  const edge = (name, w) => w > 0 ? `border-${name}:${w}px solid` : `border-${name}:none`;
  return [edge("top", b.top), edge("right", b.right), edge("bottom", b.bottom), edge("left", b.left)].join(";");
}
function tableToHtml(table) {
  ...
  if (cw && cw.length === numCols && cw.every((w) => w > 0)) {
    const total = cw.reduce((s, w) => s + w, 0);
    if (total > 0) {
      const colTags = cw.map((w) => `<col style="width:${(100 * w / total).toFixed(1)}%">`);
      lines.push(`<colgroup>${colTags.join("")}</colgroup>`);
    }
  }
  for (let r = 0; r < numRows; r++) {
    const tag = r === 0 ? "th" : "td";
    ...
    for (let c = 0; c < numCols; c++) {
      ...
      const borderStyle = cellBorderStyle(cell.border);
      if (borderStyle) attrs.push(`style="${borderStyle}"`);
...
// src/structure.ts
var ARTICLE_HEAD = /^(제\s*\d+\s*조(?:의\s*\d+)?\s*\([^)]*\))\s*(\S[\s\S]*)$/;
var REF_TAIL = /^(에|에서|을|를|와|과|및|또는|으로|로|부터|까지|이라|라는)\s/;
function splitArticleHeadings(blocks) {
  const out = [];
...
function legalStructureLevel(text) {
  const t = text.trim();
...
function detectLegalStructure(blocks) {
