...
import { splitArticleHeadings, legalStructureLevel } from "../structure.js"
// 테스트 호환을 위해 재노출 (구 import 경로 유지)
export { splitArticleHeadings, legalStructureLevel } from "../structure.js"
...
export function parseHwp5Document(buffer: Buffer, options?: ParseOptions): InternalParseResult {
  ...
  // "제N조(제목) 본문…"이 한 문단에 합쳐진 경우 → 제목(heading) + 본문(paragraph) 분리
...
/**
 * "제N조(제목) 본문…"이 한 문단에 합쳐진 경우, 제목을 heading(H3)으로 떼어내고
 * 본문은 별도 paragraph로 분리. 조문 4개 중 1개가 본문과 합쳐져 heading 누락되던
 * 문제(코퍼스 측정 75%) 해소.
 *
 * 참조 오분리 방지: 본문이 조사/접속사("에 따라", "및", "또는" 등)로 시작하면
 * 문장 중간의 조문 참조로 보고 분리하지 않는다.
 */
/**
 * 셀의 borderFillId(transient)를 docInfo.borderFills 테이블로 해석해 IRCell.border
 * (4변 px)로 변환. 별표/별지서식 양식의 무테두리/굵은선 등 원본 테두리 재현용.
 * 해석 후 borderFillId는 제거한다.
 */
function applyCellBorders(blocks: IRBlock[], docInfo: HwpDocInfo): void {
...
function detectHwp5Headings(blocks: IRBlock[], docInfo: HwpDocInfo): void {
  ...
  for (const block of blocks) {
    ...
    // 법령 구조 패턴 (스타일 유무 무관) — outlineLevel 없는 문단도 일관 승격
...
function parseCellBlock(records: HwpRecord[], startIdx: number, tableLevel: number, counter?: { count: number }) {
  ...
  // HWP5 셀 LIST_HEADER: colAddr@8, rowAddr@10, colSpan@12, rowSpan@14,
  // width(u32)@16, height(u32)@20 (HWPUNIT). 실측 덤프로 확인된 레이아웃.
  if (rec.data.length >= 16) {
    ...
    if (rec.data.length >= 20) {
      ...
      if (w > 0 && w < 1_000_000) width = w // HWPUNIT, 비정상값 방어
    }
    // borderFillID: margins(4×u16 @24-30) 뒤 @32 (실측 확인). 1-based 인덱스.
  ...
  flushText()
  return { cell: { text: texts.join("\n"), colSpan, rowSpan, colAddr, rowAddr, width, borderFillId } as CellContext, nextIdx: i }
}

/**
 * 셀 width(HWPUNIT)로 컬럼별 너비 배열 계산. 원본 표의 열 비율 재현용.
 * colSpan=1 셀에서 직접 채우고, 빈 컬럼은 병합 셀 너비를 균등 분배.
 * 모든 컬럼을 채우지 못하면 null(렌더러는 colWidths 없으면 자동 폭 사용).
 */
function computeColWidths(cols: number, cells: CellContext[]): number[] | null {
  const w = new Array<number>(cols).fill(0)
  // 1) 단일 컬럼 셀에서 직접
  for (const c of cells) {
    if (c.colAddr === undefined || !c.width || c.colSpan !== 1) continue
    if (c.colAddr >= 0 && c.colAddr < cols && w[c.colAddr] === 0) w[c.colAddr] = c.width
  }
  // 2) 남은 컬럼은 병합 셀 너비를 균등 분배
  for (const c of cells) {
    if (c.colAddr === undefined || !c.width || c.colSpan <= 1) continue
