...
import { detectLegalStructure } from "../structure.js"
...
/** getDocument + 타임아웃 래퍼 */
/**
 * pdfjs-dist 정리 호출을 버전 안전하게 감싼다.
 * pdfjs v5/v6에서 정리 API 시그니처가 바뀌어 `destroy`가 그대로 존재하지 않을 수 있다.
 * 메서드가 없으면 `doc.destroy is not a function` 동기 TypeError가 던져져
 * (`.catch()`로도 못 막음) 정상 파싱 결과까지 PARSE_ERROR로 가려진다.
 */
export async function safeDestroy(o: unknown): Promise<void> {
  try {
    const obj = o as { destroy?: () => unknown; cleanup?: () => unknown } | null
    if (obj && typeof obj.destroy === "function") await obj.destroy()
    else if (obj && typeof obj.cleanup === "function") await obj.cleanup()
  } catch {
    /* ignore — 정리 실패는 파싱 결과에 영향 주지 않음 */
  }
}
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => { void safeDestroy(loadingTask); reject(new KordocError("PDF 로딩 타임아웃 (30초 초과)")) }, PDF_LOAD_TIMEOUT_MS)
      }),
    ])
  } finally {
...
export async function parsePdfDocument(buffer: ArrayBuffer, options?: ParseOptions): Promise<InternalParseResult> {
  ...
  try {
    ...
    // □/■ 마커 기반 서브헤딩 감지 (ODL 패턴) + 법령 구조(조문/별표/별지/부칙) 공용 후처리
    detectMarkerHeadings(blocks)
    const structured = detectLegalStructure(blocks)

    // outline 구축
    const outline: OutlineItem[] = structured
...
/**
 * 셀 내부 숫자 공백 복원 (P3). 한컴 계열 PDF의 자간 균등배분이 숫자/쉼표 사이에
 * 공백을 흩뿌리는 문제("176 , 822", "- 58 , 816")를 셀 단위로만 보정한다.
 * 본문 산문("2026년 12월 31일", "제363조" 등)에 영향을 주지 않도록 표 셀 경로에서만 호출.
 *
 * - 숫자 사이 쉼표 양옆 공백 제거: 176 , 822 → 176,822 (연쇄 그룹 포함)
 * - 셀 머리/여는 괄호 뒤 단독 음수 부호 + 공백 제거: - 58,816 → -58,816
 *
 * 천 단위 무쉼표 병합(176 822 → 176822)은 "100 200 300"처럼 분리된 값을 잘못
 * 붙일 위험이 커서 적용하지 않는다 (쉼표가 있는 케이스만 안전하게 처리).
 */
export function collapseNumberSpacing(text: string): string {
  return text
    .replace(/(\d)\s*,\s*(?=\d)/g, "$1,")
    .replace(/(^|\()\s*[-−]\s+(?=\d)/g, "$1-")
}
 ...
 */
/**
 * 신구조문대비표(국회 일부개정법률안 등) 전용 검출·2단 재구성.
 *
 * "현 행 | 개 정 안" 2단 비교표는 중앙정렬 헤더의 큰 자간 때문에 일반 표 감지에서
 * 3+열로 깨지고 좌우가 섞인다. 표준 포맷이므로 중앙 거터로 좌(현행)/우(개정안)를
 * 가르고 Y밴드로 정렬해 깨끗한 2열 표로 재구성한다. 시그니처 없으면 null.
 */
export function detectAmendmentTable(items: NormItem[], pageNum: number): IRBlock[] | null {
  if (items.length < 8) return null
  const allText = items.map(i => i.text).join("")
  const hasTitle = /신.{0,2}구조문\s*대비표/.test(allText)
  const hasHeader = /현\s*행/.test(allText) && /개\s*정\s*안/.test(allText)
  // 마커(생략/현행과같음/긴 대시)는 단독으론 약함 → 조문(제N조)이 함께 있을 때만 인정
  const hasMarkers = (/\(생\s*략\)|현행과\s*같음|------/.test(allText)) && /제\s*\d+\s*조/.test(allText)
  if (!hasTitle && !hasHeader && !hasMarkers) return null

  let minX = Infinity, maxX = -Infinity
  for (const it of items) { if (it.x < minX) minX = it.x; const r = it.x + it.w; if (r > maxX) maxX = r }
  const W = maxX - minX
  if (W <= 0) return null

  const gutter = findCenterGutter(items, minX, maxX, W)
  if (gutter === null) return null

  const sorted = [...items].sort((a, b) => b.y - a.y || a.x - b.x)
  const bands = groupByY(sorted)
  const rows: IRCellLite[][] = []
  let headerSeen = false
  for (const band of bands) {
    const text = band.map(i => i.text).join("")
    if (/신.{0,2}구조문\s*대비표/.test(text)) continue // 제목 스킵
    const l = band.filter(i => i.x + i.w / 2 < gutter).sort((a, b) => a.x - b.x)
    const r = band.filter(i => i.x + i.w / 2 >= gutter).sort((a, b) => a.x - b.x)
    const lt = l.map(i => i.text).join(" ").replace(/\s+/g, " ").trim()
    const rt = r.map(i => i.text).join(" ").replace(/\s+/g, " ").trim()
    if (!lt && !rt) continue
    // 헤더행("현 행" / "개 정 안") → 정규화
    if (!headerSeen && /현\s*행/.test(lt) && /개\s*정\s*안/.test(rt)) {
      rows.push([{ text: "현행" }, { text: "개정안" }])
      headerSeen = true
      continue
    }
    rows.push([{ text: lt }, { text: rt }])
  }
  if (rows.length < 2) return null

  const cells = rows.map(r => r.map(c => ({ text: c.text, colSpan: 1, rowSpan: 1 })))
  const table: IRTable = { rows: cells.length, cols: 2, cells, hasHeader: headerSeen }
  return [{ type: "table", table, pageNumber: pageNum, bbox: computeBBox(items, pageNum) }]
}

interface IRCellLite { text: string }

/** 중앙 부근의 컬럼 거터 X 추정 (신구조문대비표 좌/우 분리용) */
function findCenterGutter(items: NormItem[], minX: number, maxX: number, W: number): number | null {
  // 시작 x 히스토그램 (10px bin)
  const bins = new Map<number, number>()
  for (const i of items) { const b = Math.round(i.x / 10) * 10; bins.set(b, (bins.get(b) || 0) + 1) }
  // 좌측 모드 = 최소 x 근처 최빈, 우측 모드 = 중앙(0.35W) 이후 최빈
  const leftMode = minX
  const rightCand = [...bins.entries()].filter(([x]) => x > minX + W * 0.35).sort((a, b) => b[1] - a[1])[0]
  if (!rightCand || rightCand[1] < 3) return null
  const rightMode = rightCand[0]
  if (rightMode <= leftMode + W * 0.2) return null
  // 좌측 컬럼의 최대 오른쪽 끝 (우측 모드 왼쪽)
  let leftRight = leftMode
  for (const i of items) {
    const r = i.x + i.w
    if (i.x < rightMode - 5 && r > leftRight && r < rightMode) leftRight = r
  }
  return (leftRight + rightMode) / 2
}
...
function extractPageBlocksWithLines(
  ...
  // 신구조문대비표(일부개정법률안) 전용 2단 재구성 — 일반 표 감지보다 우선
  const amend = detectAmendmentTable(items, pageNum)
...
function extractBlocksWithGrids(
  ...
  for (const grid of sortedGrids) {
    ...
    for (const cell of cells) {
      ...
      // 셀 텍스트 균등배분 공백 제거 ("경 제 총 괄 반" → "경제총괄반") + 숫자 공백 복원
      text = text.split("\n").map(line => collapseNumberSpacing(collapseEvenSpacing(line))).join("\n")
  ...
  if (remaining.length > 0) {
    ...
    // Y좌표 기반 정렬: 위→아래, 같은 Y(상단 동일)면 좌→우 (P1, 좌우 나란한 표/단)
    blocks.sort((a, b) => {
      const ay = a.bbox ? (a.bbox.y + a.bbox.height) : 0
      const by = b.bbox ? (b.bbox.y + b.bbox.height) : 0
      const ax = a.bbox ? a.bbox.x : 0
      const bx = b.bbox ? b.bbox.x : 0
      return (by - ay) || (ax - bx) // PDF는 y가 위가 큼 → 내림차순, 동률이면 x 오름차순
    })
...
/**
 * 두 bbox의 X구간 겹침 비율(좁은 쪽 기준). 좌우 나란히 배치면 0에 가깝다.
 * bbox가 없으면 판단 불가 → 1(겹침으로 간주, 기존 동작 보존).
 */
function xOverlapRatio(a?: BoundingBox, b?: BoundingBox): number {
  if (!a || !b) return 1
...
/** 같은 열 수 + X구간이 겹치는 연속 테이블 블록을 하나로 합침 (세로 연속 표) */
export function mergeAdjacentTableBlocks(blocks: IRBlock[]): IRBlock[] {
  if (blocks.length <= 1) return blocks
  const result: IRBlock[] = [blocks[0]]
  for (let i = 1; i < blocks.length; i++) {
    const prev = result[result.length - 1]
    const curr = blocks[i]
    // X구간이 50% 미만으로 겹치면(좌우 나란한 별개 표) 병합하지 않음 (P1)
    if (prev.type === "table" && curr.type === "table" && prev.table && curr.table &&
        prev.table.cols === curr.table.cols &&
        xOverlapRatio(prev.bbox, curr.bbox) >= 0.5) {
...
function buildGridTable(lines: NormItem[][], columns: number[]): string {
  ...
  for (const row of merged) {
    for (let c = 0; c < row.length; c++) {
      if (row[c]) row[c] = collapseNumberSpacing(collapseEvenSpacing(row[c]))
    }
  }
