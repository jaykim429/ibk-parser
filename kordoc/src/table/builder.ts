...
import type { CellContext, IRBlock, IRCell, IRTable, CellBorder } from "../types.js"
...
/**
 * 셀 4변 테두리 → 인라인 CSS. 기본(전 변 1px)과 같으면 빈 문자열(표 CSS에 위임).
 * 무테두리(0) 변이나 굵은 선만 명시 → 별표/별지서식 양식의 열린 칸·굵은선 재현.
 */
function cellBorderStyle(b?: CellBorder): string {
  if (!b) return ""
  if (b.top === 1 && b.right === 1 && b.bottom === 1 && b.left === 1) return ""
  const edge = (name: string, w: number) => w > 0 ? `border-${name}:${w}px solid` : `border-${name}:none`
...
function tableToHtml(table: IRTable): string {
  ...
  // 원본 컬럼 너비 비율 재현 (HWP5 colWidths) — colgroup으로 출력
