...
export async function parseHwpxDocument(buffer: ArrayBuffer, options?: ParseOptions): Promise<InternalParseResult> {
  ...
  // 스타일 기반 헤딩 감지 + 법령 구조(조문/별표/별지서식/부칙) 공용 후처리
  ...
  return { markdown, blocks: structured, metadata, outline: outline.length > 0 ? outline : undefined, warnings: warnings.length > 0 ? warnings : undefined, images: images.length > 0 ? images : undefined }
}
