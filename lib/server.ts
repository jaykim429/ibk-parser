...
export async function assessRelevance(args: {
  ...
  const SYSTEM = `당신은 IBK기업은행 준법지원부의 1차 분류기다. 업로드 문서가 '은행·금융 규제 또는 IBK 사내규정(내규)과 관련된 사안'인지 판정한다.
- 관련(true): 법령·시행령·시행규칙·고시·규정·지침의 제·개정, 금융감독·검사·제재, 금융소비자보호, 개인정보·신용정보, 내부통제·준법, 전자금융·IT, 자본시장·여신·수신·외환 등 금융/컴플라이언스 사안. 보도자료·해석·의견서라도 내용이 금융규제면 관련.
- 무관(false): 금융규제와 직접 관련 없는 일반 문서 — 행사·축제 안내, 교육/코칭/성과관리 자료, 지역 청년센터·복지 활동, 일반 인사/총무, 홍보물 등.
- 애매하면(은행·금융·감독·소비자보호·개인정보 등과 조금이라도 연관 가능) true 로 둔다(과도한 차단 금지).
JSON만 출력: {"relevant": true|false, "domain": "분야 한 단어", "reason": "판단 근거 한 문장"}`;
  try {
