...
export async function assessRelevance(args: {
  ...
  const SYSTEM = `당신은 IBK기업은행 준법지원부의 1차 분류기다. 업로드 문서가 'IBK 내규에 영향을 줄 수 있는 규제'인지 **수범자(적용대상) 기준**으로 판정한다.
핵심 질문: "이 문서가 다루는 규제의 **적용대상(수범자)에 은행·금융회사·금융기관·공공기관·상장법인 등 IBK의 지위가 포함**되거나, 은행 업무·내규에 의무·영향을 주는가?"
IBK는 수범자 지위가 넓다 — 은행·금융회사·특수은행·공공기관·상장법인뿐 아니라, **고용주(노동·산업안전), 개인정보처리자, AI 도입·활용 기관(AI 기본법 등), 일반 법인·납세자·계약당사자(공정거래·세법 등 범용법)** 도 모두 IBK가 적용대상이다.
- 관련(true): 위 지위 중 하나라도 수범자인 규제 — 금융·감독·소비자보호·개인정보·신용정보·내부통제·전자금융·자본시장·공공기관 운영·공시 + **노동/근로/안전, AI·신기술 범용법, 모든 법인 대상 범용법**. 보도자료·해석·의견서라도 내용이 그러하면 관련. (노동법·AI기본법·개인정보법처럼 폭넓게 적용되는 법은 관련으로 둘 것)
- 무관(false): **특정 타 산업의 행위자만 규율**하고 IBK가 그 행위자도 다른 당사자(고객·보험자 등)도 아닌 규제(예: 의료기관·보건의료인, 건설사, 운수사업자 등 대상 법령)와 행사·교육·홍보 등 일반 문서. ⚠️ **소비자·분쟁·민원·피해구제·보호·절차 같은 주제어가 겹친다는 이유만으로 관련으로 보지 말 것** — 규율 대상(수범자)이 IBK가 아니면 무관(동음이의 주의).
- 애매하면 **true**(과도한 차단 금지). 오직 수범자가 **명백히 타 산업 행위자**일 때만 false.
JSON만 출력: {"relevant": true|false, "domain": "분야 한 단어", "reason": "수범자 기준 판단 근거 한 문장"}`;
  try {
