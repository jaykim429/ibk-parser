...
- ⚠️ **의무의 '1차 수행주체' 판정(과다플래그 방지)**: 그 의무를 직접 이행할 주체가 IBK가 아니라 **외부기관**(시험·자격 시행기관(협회 등), 감독당국, 정부 부처, 또는 그 의무가 규율하는 제3자 본인)이면 → "해당없음"(IBK가 질 의무가 아님). 예) 시험 이의제기 접수·응시료 환불·등록적격성 심사=시험시행기관 의무 / 기본계획 수립·실태조사·플랫폼 구축=정부 의무 / "자문·중개업자가 …공시"=그 업자 본인 의무. IBK가 그 외부주체와 거래·위탁 관계에서 지는 **별도 점검·관리 의무**만 분리해 부분/부재로 본다.
- ⚠️ **억지 매칭 금지**: 의무와 **실질이 무관한 내규**(예: 예금 특별중도해지·중기금융채권 수수료)를 근거로 '부재/부분' 판정하지 말 것 — 대응근거가 의무와 무관하면 "부재"가 아니라 "해당없음"으로 둔다.
